...
class CutsceneDialogueSL extends FlxSpriteGroup
{
	...
	public function new(state:PlayState)
	{
		...
		var centerText:Alphabet = new Alphabet(0, FlxG.height / 2, 'BF is missing, GF went out by herself looking for him worried sick...', false, true,
			false, 1);
		...
		// lmfao
		FlxTween.tween(centerText, {alpha: 1}, 0.75, {
			onComplete: function(tween:FlxTween)
			{
				var newTimer:FlxTimer = new FlxTimer().start(5, function(timer:FlxTimer)
				{
					FlxTween.tween(centerText, {alpha: 0}, 0.75, {
						onComplete: function(tween:FlxTween)
						{
							centerText.kill();
							centerText.destroy();
					...
					FlxTween.tween(blackBackground, {alpha: 0.5}, 2, {
						onComplete: function(tween:FlxTween)
						{
							FlxTween.tween(gf, {alpha: 1}, 0.25);
							FlxTween.tween(box, {alpha: 1}, 0.25, {
								onComplete: function(tween:FlxTween)
								{
									gf.animation.play('call');
	...
	override public function update(elapsed:Float)
	{
		...
		if (startText)
		{
			...
			else if (text != null && text.swagTypingTimer != null)
	...
	final textArray:Array<String> = [
		...
		"You're looking for a blue dwarf, child?",
		"Follow me into the forest, I saw him in\nthere..."
	];
		...
		if (text != null)
		{
			if (text.finishedLine && FlxG.keys.justPressed.ANY)
			{
				...
				if (textProgression > textArray.length)
				{
					...
					FlxTween.tween(gf, {alpha: 0}, 0.25, {
						onComplete: function(tween:FlxTween)
						{
			...
			else if (text.swagTypingTimer != null && canAccelerate)
