...
class UnlockSubstate extends MusicBeatSubState
{
	...
	public function new(?unlockableName:String)
	{
		...
		FlxTween.tween(textbox, {alpha: 1, boxWidth: 24, boxHeight: 12}, 0.75, {
			...
			onComplete: function(tween:FlxTween)
			{
				...
				unlockText.forEach(function(alphaChar:AlphaCharacter)
				{
					...
					FlxTween.tween(alphaChar, {"scale.x": 1, "scale.y": 1}, 2.25, {
						ease: FlxEase.elasticOut,
				...
				FlxTween.tween(textbox, {y: textbox.y + 80,}, 0.75, {
			...
			if (controls.ACCEPT)
			{
				...
				if (unlockText != null)
				{
					unlockText.forEach(function(alphaChar:AlphaCharacter)
					{
						...
						FlxTween.tween(alphaChar, {"scale.x": 1.25, "scale.y": 1.25, alpha: 0}, 0.5, {
...
class LockSprite extends FNFSprite
{
	...
	public function unlock()
	{
		...
		new FlxTimer().start(0.1, function(timer:FlxTimer)
		{
			...
			FlxTween.tween(newLock, {"scale.x": 1.5, "scale.y": 1.5, alpha: 0}, 0.25, {
				onComplete: function(tween:FlxTween)
				{
					newLock.kill();
					newLock.destroy();
